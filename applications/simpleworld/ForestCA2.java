...
public class ForestCA2 extends CellularAutomataInteger {

	CellularAutomataDouble _cellsHeightValuesCA;
	
	World world;
	
	public ForestCA2 ( World __world, int __dx , int __dy, CellularAutomataDouble cellsHeightValuesCA )
	{
    		...
    		for ( int y = 0 ; y != _dy ; y++ )
    		{
    			if ( _cellsHeightValuesCA.getCellState(x,y) >= 0.1 * world.getMaxEverHeight() && _cellsHeightValuesCA.getCellState(x,y) <= 0.35*world.getMaxEverHeight())
    			{
    				if ( Math.random() < 0.53 ) // was: 0.71
    					...
    					this.setCellState(x, y, 12); // empty
    			}
    			...
    			/*if ( _cellsHeightValuesCA.getCellState(x,y) >= 0.2 * world.getMaxEverHeight()
    					&& _cellsHeightValuesCA.getCellState(x,y) <= 1*world.getMaxEverHeight())
    			{
    				if ( Math.random() >= _cellsHeightValuesCA.getCellState(x,y) * 40*world.getMaxEverHeight() ) // was: 0.71
    					this.setCellState(x, y, 1); // tree
    				else
